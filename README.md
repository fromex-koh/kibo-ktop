...
| 변수                         | 필수    | 설명                                                                                                                          |
| ---------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_SITE_URL`       | 배포 시 | 사이트 절대 URL. 메타데이터(OG)·`robots.txt`·canonical 의 절대경로 생성에 사용. 미설정 시 `https://kibo-ktop.vercel.app` 폴백 |
| `NEXT_PUBLIC_REPOSITORY_URL` | 선택    | 퍼블리싱 인덱스에서 표시할 저장소 URL. 미설정 시 `https://github.com/fromex-koh/kibo-ktop` 폴백                               |
