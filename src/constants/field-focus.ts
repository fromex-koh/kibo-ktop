...
//
// 자식 컨트롤이 키보드 포커스(focus-visible)되면 wrapper 전체를 점선 outline 이 감싸고
...
//
// 색·두께·offset 은 컴포넌트 focus 와 동일(outline-ring / outline-2 / outline-offset-2), 점선(outline-dotted).
// outline 모서리는 각지게 둔다(rounded 미적용).
export const FIELD_FOCUS_RING =
    'has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-dotted has-[:focus-visible]:outline-ring has-[:focus-visible]:outline-offset-2 [&_:focus-visible]:outline-none'
