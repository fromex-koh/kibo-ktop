...
const REQUIRED_PATHS = [
    ...
    },
    {
        ...
        description: '라이트·다크 테마 적용을 위한 Provider',
    },
    {
...
const TECH_STACK = [
    ...
    ['UI', 'React 19 · shadcn/ui · Radix UI · Base UI'],
...
const PathList = ({items}: {items: typeof REQUIRED_PATHS}) => (
    <dl className="divide-border divide-y">
        {items.map((item) => (
            ...
            >
                <dt>
                    <code className="text-primary-strong font-mono text-sm font-semibold">{item.path}</code>
                </dt>
        ...
        <section className="flex flex-col gap-6" aria-labelledby="project-introduction-title">
            ...
            <BaseCard>
                <p className="typo-body-l-regular text-foreground-subtle">
                    디자인 값을 토큰으로 관리하고, shadcn 기반 공통 컴포넌트와 실제 사용 예시를 함께 제공합니다. 다음
                    작업자는 가이드에서 컴포넌트의 variant·size·상태·폼 동작을 확인한 뒤 화면을 조합할 수 있습니다.
                </p>
            </BaseCard>
        </section>
        ...
        <section className="flex flex-col gap-6" aria-labelledby="core-principles-title">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                {CORE_PRINCIPLES.map((principle) => (
                    <BaseCard key={principle.title} title={principle.title}>
...
└── vendor/shadcn-baseline/      # 바닐라 CVA 비교 기준선`}</code>
        ...
        <section className="flex flex-col gap-6" aria-labelledby="ui-architecture-title">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <BaseCard title="ui">
                    <p className="text-foreground-subtle">primitive의 구조·동작·접근성을 담당하는 기반 계층</p>
                </BaseCard>
                <BaseCard title="theme">
                    <p className="text-foreground-subtle">프로젝트 CVA, variant와 상태별 시각 스타일을 담당</p>
                </BaseCard>
                <BaseCard title="composite">
                    <p className="text-foreground-subtle">ui를 import해 조합하고 프로젝트 API가 필요할 때 확장</p>
                </BaseCard>
                <BaseCard title="custom">
                    <p className="text-foreground-subtle">적합한 primitive가 없는 프로젝트 고유 UI를 구현</p>
                </BaseCard>
            </div>
            <BaseCard>
                <p className="text-foreground text-center font-mono font-semibold">
        ...
        <section className="flex flex-col gap-6" aria-labelledby="migration-required-title">
            <SectionHeader>
                ...
                <SectionHeaderDescription>
                    아래 경로는 서로 import로 연결되어 있으므로 사용하려는 컴포넌트의 의존 관계를 따라 함께 복사합니다.
                </SectionHeaderDescription>
            </SectionHeader>
            <BaseCard>
        ...
        <section className="flex flex-col gap-6" aria-labelledby="migration-token-title">
            <SectionHeader>
                ...
                <SectionHeaderDescription>
                    프로젝트 디자인 값은 아래 파일을 거쳐 생성됩니다. 하나라도 빠지면 시맨틱 유틸리티가 정상 동작하지
                    않습니다.
                </SectionHeaderDescription>
            </SectionHeader>
            <BaseCard>
        ...
        <section className="flex flex-col gap-6" aria-labelledby="migration-check-title">
            <SectionHeader>
                ...
                <SectionHeaderDescription>
                    가이드 페이지 자체는 실행에 필요하지 않습니다. 실제 컴포넌트를 대상 프로젝트 화면에 렌더링한 뒤 아래
                    순서로 확인합니다.
                </SectionHeaderDescription>
            </SectionHeader>
            <BaseCard>
