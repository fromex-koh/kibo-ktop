        ...
        <BaseCard>
            <section aria-labelledby="checkbox-state" className="flex flex-col gap-4">
                <div>
                    ...
                    <p className="typo-body-l-regular text-muted-foreground">
                        선택(off/on/indeterminate) × 상태(default/disabled) 조합입니다. Indeterminate는 하위 항목이
                        일부만 선택된 상태를 나타냅니다. 체크됨은 <code className="font-mono">blue.500</code>, 비활성은
                        회색 박스(
            ...
            <section aria-labelledby="checkbox-depth" className="flex flex-col gap-4">
                <div>
                    ...
                    <p className="typo-body-l-regular text-muted-foreground">
                        ...
                        <code className="font-mono">FieldLabel</code> (+{' '}
                        <code className="font-mono">FieldDescription</code>) 조합으로 두 형태를 만듭니다.
                    </p>
                </div>
            ...
            <section aria-labelledby="checkbox-form" className="flex flex-col gap-4">
                <div>
                    ...
                    <p className="typo-body-l-regular text-muted-foreground">
                        ...
                        <code className="font-mono">name</code>과 <code className="font-mono">value</code>를 지정하면
                        체크된 값만 제출됩니다. 같은 <code className="font-mono">name</code>을 사용하는 복수 값은{' '}
                        <code className="font-mono">FormData.getAll()</code>로 읽습니다. Indeterminate 전체 선택
                        Checkbox는 하위 항목을 제어하는 요약 상태이므로 <code className="font-mono">name</code>을 주지
                        않습니다. 실제 FormData에는 선택된 하위 Checkbox 값만 포함됩니다.{' '}
                        ...
                        <code className="font-mono">disabled</code> 항목은 제출에서 제외됩니다. 필수 약관을 선택하지
                        않고 제출하면 첫 번째 오류 Checkbox로 포커스를 이동합니다. 1depth는 라벨 아래에, 2depth는 설명
                    ...
                    <div className="bg-surface border-border mt-3 flex flex-col gap-1 rounded-md border p-4">
                        ...
                        <p className="typo-body-l-regular text-muted-foreground">
                            ...
                            제외됩니다. 실제 조작 요소인{' '}
                            <code className="font-mono">button[role=&quot;checkbox&quot;]</code>은 연결된 Label을 통해
                            접근 가능한 이름을 제공하므로 실제 사용자 접근성에 영향을 주지 않는 자동 검사 오탐으로
                            판단하여 예외 처리합니다.
                        </p>
                    </div>
                </div>
