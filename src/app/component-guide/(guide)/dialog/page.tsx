...
const PROPS_ITEMS = [
    ['Dialog', '루트. open·onOpenChange 로 제어(비제어면 Trigger 로 자동).', 'Root'],
    ['DialogTrigger', '여는 버튼. asChild 로 kit Button 을 감싼다.', 'asChild'],
    ['DialogContent', '카드. showCloseButton(기본 true)·className(max-w 등).', 'showCloseButton'],
    ['DialogHeader / Title / Description', '헤더 묶음. Title=24px Bold, Description=본문 색.', '-'],
    ['DialogFooter', '버튼 영역(가운데 정렬). showCloseButton 으로 기본 닫기 버튼.', 'showCloseButton'],
    ['DialogClose', '닫기 트리거. asChild 로 취소 버튼 등을 감싼다.', 'asChild'],
] as const

// 다이얼로그 — shadcn Dialog 를 Figma 팝업 스타일로 재스킨한 styled copy(kit). radix 가 포커스 트랩·Esc·
// 포커스 복귀·배경 스크롤 잠금을 담당하므로 접근성은 그대로 신뢰한다([8.2.1]).
    ...
    >
        <section aria-labelledby="dialog-usage" className="flex flex-col gap-4">
            <div>
                ...
                <p className="typo-body-l-regular text-muted-foreground">
                    <code className="font-mono">Dialog</code> 안에 <code className="font-mono">DialogTrigger</code>(여는
                    버튼)와 <code className="font-mono">DialogContent</code>(카드)를 둡니다. 카드는{' '}
                    <code className="font-mono">Header</code>·<code className="font-mono">Footer</code> 로 구성합니다.
                </p>
            </div>
                        ...
                        <div className="text-label-foreground max-h-64 overflow-y-auto pr-2 text-base">
                            {Array.from({length: 12}, (_, i) => (
                                <p key={i} className="mb-3">
                                    ...
                                    특정 목적 적합성을 보증하지 않습니다. 이용자는 결과를 최종 의사결정의 유일한 근거로
                                    삼지 않아야 합니다.
                                </p>
                            ))}
                        </div>
                ...
                <Dialog>
                    ...
                    <DialogContent showCloseButton={false}>
                        <DialogHeader>
                            ...
                            <DialogDescription>우상단 X 없이, 아래 버튼으로만 선택할 수 있습니다.</DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            <DialogClose asChild>
                                <Button variant="tertiary" size="xlarge">
        ...
        <section aria-labelledby="dialog-a11y" className="flex flex-col gap-4">
            ...
            <ul className="typo-body-l-regular text-muted-foreground flex list-disc flex-col gap-2 pl-5">
                <li>radix 가 포커스 트랩·Esc 닫기·닫은 뒤 포커스 복귀·배경 스크롤 잠금을 제공합니다([8.2.1]).</li>
                <li>
        ...
        <section aria-labelledby="dialog-props" className="flex flex-col gap-4">
            <div>
                ...
                <p className="typo-body-l-regular text-muted-foreground">Dialog 를 이루는 하위 컴포넌트입니다.</p>
            </div>
            <dl className="flex flex-col gap-2">
                {PROPS_ITEMS.map(([name, desc, extra]) => (
                    <div key={name} className="flex flex-col gap-0.5">
                        <dt className="typo-body-l-medium text-primary font-mono">
                            {name}
                            <span className="text-muted-foreground ml-2 font-normal">{extra}</span>
                        </dt>
