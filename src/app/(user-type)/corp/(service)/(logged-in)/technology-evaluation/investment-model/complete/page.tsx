...
// 기업 투자모형 (5) 완료 화면 — Figma "투자모형_4단계_제출 완료".
// 구성·문구·안내 줄 수가 Tech-Index 완료 화면과 같다(완료 알림 · 안내 두 줄 · 다음 행동) —
// 이 모형의 시안에도 은행전송·보증신청 안내가 없어 [알려드려요] 가 두 줄이다.
// 달라지는 것은 상단 제목·브레드크럼뿐이다(시안에 모형 코드 뱃지가 없다).
//
...
const CorpInvestmentModelCompletePage = () => (
    <main id="main" tabIndex={-1} className="bg-background flex min-h-0 flex-1 flex-col">
        <div className="grid-layout min-h-0 flex-1 gap-0 pt-0 pb-[clamp(--spacing(6),3.5dvh,--spacing(15))] [--viewport-fit-decorative-size:clamp(var(--spacing-viewport-fit-decorative-min),14dvh,var(--spacing-action-check))] *:col-span-full [&>:not([aria-hidden])]:shrink-0">
            ...
            {/* 시안(1080)의 100 = 9.26dvh 를 그대로 쓴다 — 안내가 두 줄이라 낮은 화면(720)에서도
                버튼까지 들어간다. KTRS-FM 완료 화면은 안내가 네 줄이라 이 비율을 낮춰 두었다. */}
            ...
            <div className="flex flex-col items-center">
                ...
                <h2 className="typo-h2-bold text-foreground text-center">평가가 완료되었어요.</h2>
            </div>
            ...
            <div className="flex flex-col gap-[clamp(--spacing(2),3.7dvh,--spacing(10))]">
                ...
                {/* 결과조회 화면이 아직 없어 링크 없이 둔다 — 만들어지면 asChild + Link 로 잇는다.
                    [메인으로 이동] 은 플랫폼 메인(/)이다 — 브레드크럼의 [홈](기업 홈, /corp/home)과 가는 곳이
                    다르다. 둘을 같은 주소로 두면 화면 안에서 같은 곳으로 가는 링크가 둘이 되어
                    (WAVE "Redundant link"), 스크린리더로 링크만 훑을 때 같은 목적지가 두 번 읽힌다. */}
                <StepNavigation
                    appearance="plain"
...
export default CorpInvestmentModelCompletePage
