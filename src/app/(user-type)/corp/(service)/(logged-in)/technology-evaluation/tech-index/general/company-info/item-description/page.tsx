...
// Tech-Index 기업정보의 [품목설명] 모달 — 화면정의서의 하위 화면이라 경로를 따로 둔다.
// 이 모달은 [기술분류] 표의 줄마다 붙는 [품목설명]이 여는 것이라 혼자 뜨는 법이 없다. 그래서 이 화면은
// 기술분류 모달을 열고 그 위에 첫 줄 품목의 설명 모달까지 겹쳐 둔다 — 실제로 보게 되는 모습 그대로다.
// 뒤 배경을 비우는 것은 다른 모달 단독 화면과 같다.
// 창업용 화면(tech-index/startup)도 같은 모달이라 그 경로는 이 화면을 다시 내보낸다.
const CorpTechIndexItemDescriptionPage = () => (
    <>
...
export default CorpTechIndexItemDescriptionPage
