...
// 기관 일괄평가 2단계 대량정보 조회 신청 입력 묶음 — Figma "[일괄평가] 2단계_대량정보 조회 신청"과
// 상태 시안 "…_파일업로드case".
// 카드 두 장이 한 폼이고, 화면 하단의 [신청]이 form 속성으로 이 폼을 제출한다.
//
// 검사를 모두 통과하면 이 화면 위에 제출 전 확인 모달("제출하시겠습니까?")을 띄우고,
// 모달의 [제출]이 (4) 대량정보 조회 신청 완료 화면으로 넘어간다 — 별도 화면으로 이동하지 않는다.
...
type BulkDataRequestFormProps = {
    // 화면 하단 [신청] 버튼이 form 속성으로 가리키는 이름.
    formId: string
    // 모달의 [제출]을 통과하면 넘어갈 (4) 대량정보 조회 신청 완료 화면 — 갈래(일반/창업)마다 경로가 다르다.
    completePath: string
}

const BulkDataRequestForm = ({formId, completePath}: BulkDataRequestFormProps) => {
    ...
    const handleExcelFileChange = (file: File | null) => {
        console.log('[프론트엔드 연동][파일 업로드] 평가내역조회 표준엑셀', file)
        setExcelFile(file)
    }
    ...
    const handleSubmit = (event: SubmitEvent<HTMLFormElement>) => {
        ...
        console.log('[기관 일괄평가] 2단계 대량정보 조회 신청 데이터', {
    ...
    const handleConfirmSubmit = () => {
        console.log('[프론트엔드 연동][제출] 기관 일괄평가 대량정보 조회 최종 제출 — 이 자리에서 제출 API 를 호출한다')
        router.push(completePath)
    }

    return (
        <form id={formId} noValidate onSubmit={handleSubmit} className="flex flex-col gap-10">
            <FormCard title="평가내역조회 목적">
                <div className="flex flex-col gap-6">
                    <Field
                        id={purposeFieldId}
                        label="사업/과제 선택"
                        required
                        error={purposeError ? '조회 목적이 되는 사업/과제를 선택해 주세요.' : undefined}
                    >
                        <Select
                            name={PURPOSE_FIELD}
                            required
            ...
            <FormCard title="평가내역조회 필수 양식">
                ...
                <div className="flex flex-col gap-10">
                    <FileUploadField
                        ...
                        error={excelError ? '평가내역조회용 표준엑셀 파일을 첨부해 주세요.' : undefined}
                    />
                    <FileUploadField
...
export {BulkDataRequestForm}
export type {BulkDataRequestFormProps}
