...
import {getOrgKBigxReportHistory} from '@/content/service/org-k-bigx-report-history'

export const metadata: Metadata = {title: 'K-BIGx 보고서 이력'}

// [프론트엔드 연동] 이 화면은 조회 결과를 받아 목록에 넘기기만 한다 — 데이터가 목업인지 API 응답인지는
// content/service/org-k-bigx-report-history.ts 가 정하므로, 연동할 때 이 파일은 고치지 않아도 된다.

// 기관 마이페이지 · K-BIGx 보고서 이력 — Figma "기관 - K-BIGx 보고서 이력" 조회결과 case.
// 조회 필터·보고서 카드·페이지 이동은 기업 화면(/corp/mypage/k-bigx-report-history)과 같은 목록 조각을 쓰고,
// 카드에 보고서 유형 배지와 조회 기관이 더해진다(값은 기관 데이터가 가져온다).
// 두 열 배치·간격은 마이페이지의 다른 화면과 같다(사이드바 344 + 64 + 본문 792 = 1200).
const OrgMypageKBigxReportHistoryPage = async () => {
    const reports = await getOrgKBigxReportHistory()
                ...
                <div className="flex flex-col gap-10 pb-15 xl:flex-row xl:gap-16">
                    ...
                    <div className="flex min-w-0 flex-1 flex-col gap-10">
                        ...
                        <section aria-labelledby="k-bigx-report-list-title" className="flex flex-col">
                            ...
                            <KBigxReportHistoryList
                                items={reports}
                                pageSize={K_BIGX_REPORT_HISTORY_PAGE_SIZE}
                                userType="org"
                            />
                        </section>
                    </div>
...
export default OrgMypageKBigxReportHistoryPage
