import type {Metadata} from 'next'
import {
    Breadcrumb,
    BreadcrumbItem,
    BreadcrumbLink,
    BreadcrumbList,
    BreadcrumbPage,
} from '@/components/composite/breadcrumb'
import {BreadcrumbDotSeparator} from '@/components/composite/breadcrumb-dot-separator'
import {MypageSidebar} from '@/components/composite/mypage-sidebar'
import {PageTitleBar} from '@/components/composite/page-title-bar'
import {SectionHeader, SectionHeaderDescription, SectionHeaderTitle} from '@/components/composite/section-header'
import {InquiryList, type InquiryItem} from '@/components/custom/inquiry-list'
import {ORG_MYPAGE_MEMBERS} from '@/constants/mypage-profile'

export const metadata: Metadata = {title: '1:1 문의 내역'}

// [프론트엔드 연동] 이 화면이 쓰는 값은 모두 여기 세 상수뿐이고, 목록 컴포넌트(InquiryList)는
// 넘겨받은 것만 그린다 — 조회 코드를 화면 안에서 찾아다닐 필요가 없다.
//   · MOCK_INQUIRY_ITEMS   → 문의 목록 조회 응답으로 교체한다. status 는 'waiting'(답변대기) ·
//                            'answered'(답변완료) 두 가지이고, category 는 문의하기 화면의
//                            [유형 선택]과 같은 값이다(constants/inquiry 의 INQUIRY_TYPES).
//   · MOCK_INQUIRY_PAGE_SIZE → 한 페이지에 보여 줄 건수.
//   · 각 항목의 href → 그 문의의 상세 화면 경로다. 예제는 ?id=문의번호로 각 문의를 구분한다. 상세
//                       본문은 아직 목업이며, 실제 연동 시 해당 ID로 조회한 문의 데이터를 연결한다.
//   · INQUIRY_CREATE_PATH → [문의 등록] 이 가는 화면 경로.
// 빈 배열을 넘기면 목록 자리에 빈 상태 안내가 나온다(InquiryList 가 처리한다) — 조회 결과가 없을 때의
// 화면을 따로 만들 필요가 없고, 이 배열을 비워 보면 그대로 확인할 수 있다.
const MOCK_INQUIRY_ITEMS: readonly InquiryItem[] = [
    {
        id: 'org-inquiry-001',
        category: '기술평가',
        title: '평가검증 신청 오류 문의',
        status: 'waiting',
        date: '2026-05-15',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-001',
    },
    {
        id: 'org-inquiry-002',
        category: '기술평가',
        title: '개별평가 결과 오류 문의',
        status: 'answered',
        date: '2026-05-14',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-002',
    },
    {
        id: 'org-inquiry-003',
        category: '유료/결제',
        title: '이용권 배분 관련 문의',
        status: 'answered',
        date: '2026-05-12',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-003',
    },
    {
        id: 'org-inquiry-004',
        category: '회원정보',
        title: '하위 계정 생성 문의',
        status: 'waiting',
        date: '2026-05-11',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-004',
    },
    {
        id: 'org-inquiry-005',
        category: '기술평가',
        title: '일괄평가 신청 방법 문의',
        status: 'answered',
        date: '2026-05-08',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-005',
    },
    {
        id: 'org-inquiry-006',
        category: 'K-BIGx',
        title: 'K-BIGx 보고서 열람 권한 문의',
        status: 'answered',
        date: '2026-05-07',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-006',
    },
    {
        id: 'org-inquiry-007',
        category: '기타',
        title: '협약 기관 등록 절차 문의',
        status: 'answered',
        date: '2026-05-04',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-007',
    },
    {
        id: 'org-inquiry-008',
        category: '회원정보',
        title: '담당자 변경 절차 문의',
        status: 'answered',
        date: '2026-04-29',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-008',
    },
    {
        id: 'org-inquiry-009',
        category: '유료/결제',
        title: '이용권 잔여 건수 문의',
        status: 'answered',
        date: '2026-04-27',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-009',
    },
    {
        id: 'org-inquiry-010',
        category: '기술평가',
        title: '평가검증 반려 사유 문의',
        status: 'answered',
        date: '2026-04-22',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-010',
    },
    {
        id: 'org-inquiry-011',
        category: '회원정보',
        title: '비밀번호 초기화 문의',
        status: 'answered',
        date: '2026-04-18',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-011',
    },
    {
        id: 'org-inquiry-012',
        category: 'K-BIGx',
        title: '보고서 일괄 내려받기 문의',
        status: 'answered',
        date: '2026-04-15',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-012',
    },
    {
        id: 'org-inquiry-013',
        category: '기타',
        title: '서비스 이용 방법 문의',
        status: 'answered',
        date: '2026-04-11',
        href: '/org/mypage/inquiry-history/inquiry-detail?id=org-inquiry-013',
    },
]

// 한 페이지 10건 — 목업이 13건이라 2페이지가 만들어진다(페이지 이동 확인용).
const MOCK_INQUIRY_PAGE_SIZE = 10

const INQUIRY_CREATE_PATH = '/org/notice/inquiry-create'

// 기관 마이페이지 (6) 1:1 문의 내역 — 기업 1:1 문의(/corp/mypage/inquiry-history)와 같은 화면이다.
// 다른 것은 메뉴 목록(기관 LNB)·목록 데이터·이동 경로뿐이라 목록 컴포넌트는 그대로 쓴다.
const OrgMypageInquiryHistoryPage = () => (
    <main id="main" tabIndex={-1} className="bg-background flex-1">
        <div className="grid-layout gap-y-10 pt-10 *:col-span-full">
            <PageTitleBar
                title="마이페이지"
                breadcrumb={
                    <Breadcrumb>
                        <BreadcrumbList>
                            <BreadcrumbItem>
                                <BreadcrumbLink href="/org/home">홈</BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbDotSeparator />
                            <BreadcrumbItem>
                                <span>마이페이지</span>
                            </BreadcrumbItem>
                            <BreadcrumbDotSeparator />
                            <BreadcrumbItem>
                                <BreadcrumbPage>1:1 문의 내역</BreadcrumbPage>
                            </BreadcrumbItem>
                        </BreadcrumbList>
                    </Breadcrumb>
                }
            />

            <div className="flex flex-col gap-10 pb-15 xl:flex-row xl:gap-16">
                <MypageSidebar userType="org" current="1:1 문의" companyName={ORG_MYPAGE_MEMBERS.default.companyName} />

                <div className="flex min-w-0 flex-1 flex-col gap-10">
                    <SectionHeader>
                        <SectionHeaderTitle id="inquiry-history-title" tabIndex={-1} size="lg">
                            1:1 문의 내역
                        </SectionHeaderTitle>
                        <SectionHeaderDescription size="lg">
                            문의사항을 등록하고 답변 상태를 확인할 수 있습니다.
                        </SectionHeaderDescription>
                    </SectionHeader>

                    <InquiryList
                        items={MOCK_INQUIRY_ITEMS}
                        createHref={INQUIRY_CREATE_PATH}
                        pageSize={MOCK_INQUIRY_PAGE_SIZE}
                    />
                </div>
            </div>
        </div>
    </main>
)

export default OrgMypageInquiryHistoryPage
