...
import {TECH_INDEX_MODEL_META} from '../model-meta'

// Tech-Index 평가모형 선택 — 두 모형 중 하나를 고르고 [다음]을 누른다(시안 "[혁신성장지수 평가 Tech-Index]").
// 시안의 CTA 문구는 [신청]이지만 이 화면 다음에 고객정보활용동의·입력 단계가 이어지므로 [다음]으로 둔다.
// 카드는 링크가 아니라 라디오다 — 고르는 것만으로는 넘어가지 않고, 아래 [다음]을 눌러야 다음으로 간다.
// 필수값이라 고르기 전에는 [다음]이 비활성이다.
//
// [프론트엔드 연동] 아래 console.log 자리만 신청 API 호출로 바꾸면 된다. 검사를 통과하면 고른 모형의
// (2) 고객정보활용동의로 넘어간다 — 기업처럼 라우트가 /general/ 과 /startup/ 으로 나뉜다.
...
const TechIndexModelForm = ({labelledBy, children}: TechIndexModelFormProps) => {
    ...
    const handleSubmit = (event: SubmitEvent<HTMLFormElement>) => {
        ...
        console.log('[기관 개별평가 Tech-Index] 평가모형 선택', {[MODEL_FIELD]: model})
        router.push(`${TECH_INDEX_MODEL_META[model === 'startup' ? 'startup' : 'general'].base}/customer-consent`)
    }
