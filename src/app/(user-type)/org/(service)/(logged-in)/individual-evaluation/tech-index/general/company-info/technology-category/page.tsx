...
// 기관 개별평가 Tech-Index 기업정보의 [기술분류] 모달 — 화면정의서의 하위 화면이라 경로를 따로 둔다.
// 기업 상세 정보의 기술분류 칸 옆 [조회]에서 여는 모달이고, 그 자리에도 이미 연결되어 있다.
// 내용은 기업 화면과 같다(composite/technology-category-dialog).
// 이 화면은 모달만 확인하는 자리라 뒤 배경을 비우고 모달을 열어 둔다 — 다른 모달 단독 화면과 같은 방식이다.
// 투자모형도 같은 모달이라 그 경로는 이 화면을 다시 내보낸다.
const OrgTechIndexTechnologyCategoryPage = () => (
    <>
...
export default OrgTechIndexTechnologyCategoryPage
