...
// 기관 투자모형 2단계의 입력 폼 — 화면(page)은 서버 모듈이라 "값에 따라 갈 곳을 정하는" 함수를 폼에 바로
// 넘길 수 없다(서버에서 클라이언트로 함수를 넘기지 못한다). 그 판단만 이 얇은 클라이언트 조각이 맡는다.
// 기업 투자모형 2단계의 같은 이름 조각과 하는 일이 같고, 가리키는 경로만 기관 IA 다.
//
...
const CHECKLIST_MANUFACTURING_PATH = '/org/individual-evaluation/investment-model/checklist/manufacturing'
const CHECKLIST_SERVICE_PATH = '/org/individual-evaluation/investment-model/checklist/service'
...
const ChecklistStepForm = ({formId, stickyHeader}: {formId: string; stickyHeader: ReactNode}) => (
    <SelfDiagnosisTabsForm
        ...
        tabs={ORG_INVESTMENT_MODEL_FORM_TABS}
