...
// 앞 화면 — 개별평가 > 투자모형 > (1) 고객정보활용동의.
const CONSENT_PATH = '/org/individual-evaluation/investment-model/customer-consent'
// 폼과 [다음] 버튼을 잇는 이름 — 버튼이 폼 바깥(화면 맨 아래 CTA)에 있어 form 속성으로 연결한다.
const FORM_ID = 'company-technology-info-form'

// 기관 개별평가 투자모형 (2) 기업·기술정보 입력 — 탭 여섯 개가 한 화면이고, 이 파일은 화면 뼈대
// (제목·스텝·탭·CTA)만 조립한다. 탭 본문은 composite/self-diagnosis-form-tabs 의
// ORG_INVESTMENT_MODEL_FORM_TABS 가 갖는다(지금은 [기업정보] 탭만 채워져 있고 나머지 다섯은 비어 있다).
//
// 기업 투자모형 2단계와 화면 구성이 같고 다른 것은 세 가지다 — 브레드크럼(기관 IA), 단계 이름
// (ORG_INVESTMENT_MODEL_STEPS), 그리고 기업정보 탭이 기관용(직접 입력)이라는 점이다.
const OrgInvestmentModelCompanyTechnologyInfoPage = () => (
    <main id="main" tabIndex={-1} className="bg-background flex-1">
        ...
        <div className="grid-layout gap-y-10 pt-0 *:col-span-full md:pt-10">
            ...
            <ChecklistStepForm
                formId={FORM_ID}
                stickyHeader={
                    <StepHeaderCompact title="기업·기술정보 입력" steps={ORG_INVESTMENT_MODEL_STEPS} current={2} />
                }
            />
        </div>
...
export default OrgInvestmentModelCompanyTechnologyInfoPage
