// 법인등록번호 표시 형식 — 숫자만 남기고 6-7 자리로 하이픈을 넣는다(예: 110111-1234567).
//
// 화면에서는 이 파일을 직접 쓰지 않아도 된다. composite/form-values 의 `CorporateNumberInput` 이 물고 있어
// `<CorporateNumberInput name="corpNo" />` 한 줄이면 된다(사업자번호의 `BusinessNumberInput` 과 같은 구조).
//
// 입력할 때마다 값 전체를 다시 계산하는 순수 함수다 — 상태도 DOM 도 건드리지 않으므로 그대로 단위 테스트할 수 있다.

// 자릿수 나누기 — 법인등록번호는 13자리 고정이다(앞 6자리 등기 구분 + 뒤 7자리 일련·검증).
const GROUP_SIZES = [6, 7]
const MAX_DIGIT_COUNT = GROUP_SIZES.reduce((total, size) => total + size, 0)

// 완성된 형식 — 제출 검사(input 의 pattern)와 여기가 같은 규칙을 보도록 한 곳에서 만든다.
const CORPORATE_NUMBER_PATTERN = `\\d{${GROUP_SIZES[0]}}-\\d{${GROUP_SIZES[1]}}`
...
// 사용자에게는 지우기가 먹지 않는 것처럼 보인다(사업자번호·전화번호와 같은 처리).
...
/**
 * 법인등록번호 형식으로 바꿔 돌려준다. 숫자가 아닌 글자는 버리고, 13자리를 넘으면 자른다.
 *
 * ```ts
 * formatCorporateNumber('1101111234567')   // '110111-1234567'
 * formatCorporateNumber('110111 12345')    // '110111-12345'
 * ```
 *
 ...
 */
const formatCorporateNumber = (value: string, previousValue = '') => {
...
export {CORPORATE_NUMBER_PATTERN, formatCorporateNumber}
