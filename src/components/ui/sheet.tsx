...
function SheetOverlay({className, ...props}: React.ComponentProps<typeof SheetPrimitive.Overlay>) {
    return (
        <SheetPrimitive.Overlay
            ...
            className={cn(
                // bg-black/10 → bg-overlay-sm: 이 프로젝트는 생성기가 --color-black 을 제거해 bg-black 이
                // 안 나온다. overlay 토큰(라이트=검정10% / 다크=흰10%, 다크 자동반사)으로 대체한다.
                'bg-overlay-sm data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0 fixed inset-0 z-50 duration-100 supports-backdrop-filter:backdrop-blur-xs',
                className,
            )}
            {...props}
        />
