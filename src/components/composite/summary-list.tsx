...
// 요약 목록(SummaryList) — 라벨(좌) + 값(우)으로 읽기 전용 정보를 나열하는 요약 리스트(L2 composite).
// GOV.UK Design System 의 "Summary list"(정보 확인 화면에 쓰는 항목-값 목록)와 같은 개념·이름이다.
...
// 박스 스타일만 분리 export — SelectableSummaryList 처럼 <dl> 이 아닌 다른 요소(<label>)에 똑같은
// 카드 박스(배경·테두리·라운드·패딩)를 입혀야 할 때, 클래스 문자열을 중복 작성하지 않고 그대로 재사용한다.
const summaryListBoxClassName = 'bg-surface border-subtle-3 flex w-full flex-col gap-3 rounded-md border p-6'

const SummaryList = ({className, ...props}: ComponentPropsWithoutRef<'dl'>) => (
    <dl data-slot="summary-list" className={cn(summaryListBoxClassName, className)} {...props} />
)

type SummaryListItemProps = Omit<ComponentPropsWithoutRef<'div'>, 'children'> & {
    // 라벨(왼쪽) — <dt> 로 렌더된다.
    term: ReactNode
...
export {SummaryList, SummaryListItem, summaryListBoxClassName}
