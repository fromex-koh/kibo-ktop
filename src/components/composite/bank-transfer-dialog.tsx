...
// 은행 전송 모달 — 평가결과를 받을 은행과 지점을 고른다. 완료 화면의 [은행전송] 이 연다.
// [전송하기] 를 누르면 이 모달이 닫히고 전송 결과 모달이 이어서 뜬다. 전송 API 는 handleSubmit 에 붙인다.

const BANK_FIELD = 'bank-transfer-bank'
const BRANCH_FIELD = 'bank-transfer-branch'

// 현행 서비스의 은행 목록·순서를 그대로 옮긴 것이다. 보내는 값은 은행 코드가 아니라 은행명 자체다 —
// 연동할 때 백엔드가 받는 값이 달라지지 않도록 목록을 손대지 않는다.
...
// 전송 완료 알림 — 위 전송 모달이 띄우고, 단독 확인 화면(은행 전송완료)에서도 쓴다.
const BankTransferResultDialog = ({result, defaultOpen, open, onOpenChange}: BankTransferResultDialogProps) => (
    <Dialog defaultOpen={defaultOpen} open={open} onOpenChange={onOpenChange}>
        {/* 설명 없이 알림 문구와 보낸 값뿐이라 aria-describedby 를 비운다 — 넘기지 않으면 Radix 가 경고한다. */}
        <DialogContent showCloseButton={false} aria-describedby={undefined}>
            {/* 빈 머리 구획은 지우지 않는다 — CTA 가 자동 배치라 첫 행을 비워 두면 버튼이 본문 위로 올라온다. */}
            <DialogHeader className="pb-8" />
            <div className={cn(dialogBodyClassName, 'gap-4 pt-0 pb-8')}>
                {/* 완료 아이콘 — 뜻은 아래 문구가 전하므로 장식으로 두고 읽히지 않게 한다. */}
                <span
                    aria-hidden="true"
                    className="bg-primary text-primary-foreground flex size-15 shrink-0 items-center justify-center self-center rounded-full"
                >
                    <Check className="size-icon-2xl" strokeWidth={3} />
                </span>
                <div className="flex flex-col gap-2">
                    {/* 이 문구가 곧 모달의 이름이라 DialogTitle 로 둔다 — 문단으로 바꾸면 대화상자에 이름이 없어진다. */}
    ...
    return (
        <>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                ...
                {/* 설명이 따로 없어 aria-describedby 를 비운다 — 넘기지 않으면 Radix 가 경고한다. */}
                <DialogContent aria-describedby={undefined}>
                    <DialogHeader>
                        <DialogTitle>은행 전송</DialogTitle>
                    </DialogHeader>
                    <div className={cn(dialogBodyClassName, 'gap-6')}>
                        {/* 입력 묶음을 이끄는 머리라 heading 으로 둔다. 모달 제목(h2) 아래 단계라 h3 이다. */}
