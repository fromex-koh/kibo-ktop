    ...
    return (
        <>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                ...
                <DialogContent>
                    ...
                    <div className={cn(dialogBodyClassName, 'gap-6')}>
                        {/* 무엇을 해야 하는지 알리는 문장이라 모달의 설명으로 둔다 — radix 가 aria-describedby 로 잇는다. */}
            ...
            {/* 전송 완료 — Figma "[신속표준모형 KTRS-FM] m_은행 전송완료".
                물음이 아니라 알림이라 보이는 제목과 닫기(X)를 두지 않고, 버튼은 [확인] 하나가 전체 폭을 쓴다.
            ...
            <Dialog open={!!result} onOpenChange={(open) => !open && closeResult()}>
                ...
                <DialogContent showCloseButton={false} className="min-h-68">
                    <DialogHeader>
                        <DialogTitle className="sr-only">은행 전송 완료</DialogTitle>
                    </DialogHeader>
                    {/* 알림 문구와 보낸 값 사이 8(시안). */}
                    ...
                    <DialogFooter>
                        <Button size="xl" className="w-full" onClick={closeResult}>
                            확인
                        </Button>
                    </DialogFooter>
                </DialogContent>
...
export {BankTransferDialog}
export type {BankTransferDialogProps}
