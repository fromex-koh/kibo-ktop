...
// 상태: focus(키보드)와 열림(data-[state=open]=마우스 클릭) 모두 동일한 점선 outline(outline-2 dotted,
...
function SelectTrigger({
    ...
    return (
        <SelectPrimitive.Trigger
            ...
            data-size={size}
            className={cn(
                "border-input focus-visible:outline-ring data-[state=open]:outline-ring aria-invalid:border-destructive data-placeholder:text-placeholder data-[size=default]:h-control-h-lg bg-surface aria-readonly:bg-muted disabled:bg-muted data-[size=md]:h-control-h-md flex items-center justify-between gap-1.5 rounded-sm border text-base whitespace-nowrap transition-colors outline-none select-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-dotted disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:px-4 data-[size=md]:px-4 data-[size=sm]:h-7 data-[size=sm]:rounded-[min(var(--radius-md),10px)] data-[size=sm]:px-3 data-[size=sm]:text-sm *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 data-[state=open]:outline-2 data-[state=open]:outline-offset-2 data-[state=open]:outline-dotted [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-5",
                className,
            )}
            {...props}
        >
