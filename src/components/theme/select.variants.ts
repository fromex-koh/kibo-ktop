...
const selectItemClassName =
    "text-label-foreground focus:bg-primary-subtle focus:text-label-foreground not-data-[variant=destructive]:focus:**:text-inherit data-[state=checked]:text-select-selected-foreground data-disabled:text-disabled h-control-h-lg rounded-sm relative flex w-full cursor-default items-center gap-1.5 px-2 outline-hidden typo-body-xl-regular select-none data-[state=checked]:font-medium data-disabled:pointer-events-none data-disabled:opacity-100 [&>span:first-child]:hidden [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2"
