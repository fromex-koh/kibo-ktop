...
// 표가 화면 안에 들어온다.
// 글자(typo-*)는 여기 넣지 않는다 — 제목 칸이 굵기를 다시 지정하는데, typo-* 는 twMerge 가 모르는
// 프로젝트 유틸이라 두 벌이 함께 남아 나중에 나오는 -regular 가 -bold/-medium 을 덮는다([PB-08]).
const dialogTableCellBaseClassName = 'border-subtle-3 text-foreground border px-4 py-3 align-middle whitespace-normal'

const dialogTableCellClassName = cn(dialogTableCellBaseClassName, 'typo-body-l-regular')

// 제목 칸(열 제목·분류 칸) — 옅은 파란 면에 가운데 정렬. 글자 크기·굵기는 쓰는 쪽이 정한다
// (열 제목은 Bold, 행 묶음의 분류는 Medium — 시안).
const dialogTableHeaderCellClassName = cn(dialogTableCellBaseClassName, 'bg-primary-subtle text-center')
