...
- 보완 집계: 응용2·UIUX 모두 보완을 완료 수에 포함합니다. 진척률 카드는 ‘완료(보완 포함)’로 표시하고, 집계 기준을 표 위 안내 목록에 적었습니다. 취소선 제외 기준은 그대로입니다. 최초 비밀번호 변경·실명인증 화면은 완료에서 보완으로 변경해도 완료 수에 계속 포함되어 진척률이 유지됩니다.
...
- 상태 변경: 기관 최초 비밀번호 변경(`org-initial-password-change`)과 기업 실명인증(`corp-real-name-verification`)의 UIUX 뱃지를 완료에서 보완으로 변경했습니다. 응용2는 모두 완료를 유지하며, 보완도 완료 수에 포함하므로 진척률은 유지됩니다.
