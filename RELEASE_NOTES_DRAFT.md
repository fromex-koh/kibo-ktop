...
### 퍼블리싱 인덱스 — 응용2팀 탄소중립 마이페이지 화면 상태 반영

- 대상: src/content/publishing-guide/publishing-index.json
- 적용: 응용2팀이 올린 상태값을 그대로 받은 것이라 파일을 통째로 덮어씁니다.
- 변경: 탄소중립 마이페이지 화면 12개에 응용2 상태 `완료` 가 새로 붙었습니다. 값이 없던 화면에 붙은 것이고 이미 있던 값을 고친 것은 없습니다.
    - 마이페이지 — 첫 화면
    - 하위계정 관리 — 목록 · 하위계정 등록
    - 회원정보 수정 — 기관회원 · 기업회원 · 주소검색 · 저장 확인 · 수정 취소 확인
    - 현황조회 — 목록 · 보완요청 · 내역 없음 · 삭제 확인
- 진척률: 탄소 응용2 진척률이 73%(54/74)에서 89%(66/74)로 올랐습니다. 기업·기관 화면 수치는 그대로입니다.
