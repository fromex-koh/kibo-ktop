...
- 이전: 일괄평가 (2) 아래에 일반 갈래(`/general/…`)만 있었습니다. 창업 갈래는 화면 파일이 있어 주소로 열리는데도 인덱스·경로 레지스트리 어디에도 없어, 인덱스에서 찾을 수 없고 검사 대상에서도 빠졌습니다.
- 지금: 창업 갈래 5화면을 등록했습니다 — (3) 평가내역조회 · 최종 제출 전 확인 · (4) 평가내역조회 완료 · (3) 일괄평가 진행 신청 · (4) 일괄평가 신청 완료. 같은 IA 화면의 갈래라 `iaRow` 는 일반 갈래와 같은 번호로 묶어 IA 화면 수는 늘지 않습니다.
- 함께 반영: 두 갈래를 구분하려고 기존 일반 갈래 줄에 `[일반]`, 새 줄에 `[창업]` 을 붙였습니다(예: "(3) [일반] 평가내역조회(구: 대량정보조회)").
- 유지: 창업 갈래의 일괄평가 진행 신청에는 최종 확인 모달 화면이 없어 4화면이 아니라 3화면입니다.
- 영향 화면: 퍼블리싱 인덱스 (홈) — 기관 구현 완료 화면이 136개에서 141개가 됩니다(취소선 6개 제외 기준)

### 접근성 검사 예외사항 — WAVE 화면 목록을 인덱스와 맞추고 [없음] 표시 추가

- 대상: src/app/component-guide/(guide)/accessibility-exceptions/page.tsx
- 적용: 퍼블리싱 가이드 관련 파일이라 지정한 파일만 교체합니다.
- 이전: WAVE 화면별 검사 기록이 기관 138개로, 퍼블리싱 인덱스(141개)와 어긋났습니다. 검사 기록이 없는 화면은 목록에 아예 없었습니다.
- 지금: 인덱스에는 있는데 검사 기록이 없던 일괄평가 창업 갈래 3화면(최종 제출 전 확인 · 평가내역조회 완료 · 일괄평가 신청 완료)을 목록에 넣어 기관 141개로 맞췄습니다. 세 줄은 오류·경고 칸을 한 칸으로 합쳐 [없음](검사했고 나온 오류가 없음)으로 적습니다 — 검사하지 않은 화면은 [미실행]로 적을 수 있게 줄마다 표시를 정합니다.
- 유지: 기업 136개와 다른 화면의 오류·경고 건수는 그대로입니다. W3C 마크업 검사 탭은 운영 빌드의 검사 결과를 그대로 쓰므로 이번에 손대지 않았습니다(창업 5화면이 검사에 들어오면 141개로 맞춰집니다).
