...
### [임시] 신용정보 활용체제 화면 (기업·기관)

- 대상: src/app/(user-type)/corp/(service)/(logged-out)/credit-information-policy/page.tsx
    - src/app/(user-type)/org/(service)/(logged-out)/credit-information-policy/page.tsx
- 적용: 신규 파일 추가. 다른 파일에 영향이 없습니다.
- 화면: 이용약관 화면과 같은 짜임(PageTitleBar + 임시 섹션)이고 다른 것은 제목·브레드크럼뿐입니다. 원고를 받기 전이라 본문은 `내용 추후 업데이트` 한 줄입니다.
- 확인 방법: [기업 신용정보 활용체제](/corp/credit-information-policy) `완료` · [기관 신용정보 활용체제](/org/credit-information-policy) `완료`

### [임시] 메인 공지사항 팝업 (기업·기관)

- 대상: src/app/(user-type)/corp/home-notice-popup/page.tsx
...
- 적용: 신규 파일 추가. 홈 화면(`MainPageScreen`)은 손대지 않았습니다 — 전체메뉴 화면과 같은 방식으로 그 위에 팝업만 얹습니다.
- 화면 구성: 공지 한 건이 한 장이고, 여러 건이면 옆으로 넘겨 봅니다. 장마다 분류 배지 · 게시일 · 제목 · 요약 · [자세히 보기](공지 상세로 `?id=` 링크)가 들어가고, 아래에 [오늘 하루 열지 않기]와 [닫기]가 옵니다.
- 넘기는 방식: 띠를 옆으로 밀어 보여 줍니다 — 한 장씩 갈아 끼우면 어느 쪽으로 넘어갔는지 알 수 없습니다. 화살표와 아래 점으로 넘기고, 점을 눌러 바로 그 장으로 갈 수도 있습니다.
- 저절로 넘어가지 않습니다: 스스로 움직이는 콘텐츠에는 정지 수단을 두어야 하는데[6.2.2], 읽는 속도가 사람마다 다른 공지에서는 아예 넘기지 않는 편이 낫다고 보았습니다. 동작을 줄이도록 설정한 사용자에게는 밀리지 않고 바로 바뀝니다[6.3.1].
- 접근성: 보이지 않는 장은 `inert`·`aria-hidden` 이라 탭으로 그 안의 링크가 잡히지 않고, 지금 몇 번째인지는 `aria-live` 로 알립니다(점은 장식이라 읽히지 않습니다). 포커스 트랩·Esc·배경 스크롤 잠금은 공통 모달이 맡습니다.
- 반응형: 1440 에서 588 가운데, 360 에서 좌우 16 을 남긴 312 입니다. 좁은 화면에서는 아래 두 조각이 세로로 쌓입니다.
- 연동 지점: `src/content/service/home-notices.ts` 의 `getHomeNotices()` 안을 조회 API 로 바꾸면 화면·팝업은 손댈 것이 없습니다. 빈 배열이면 팝업이 아예 뜨지 않습니다. 노출 기간·게시 여부는 서버가 걸러 준다고 보고 화면에서 다시 보지 않습니다.
- 목업으로 둔 것: [오늘 하루 열지 않기]는 지금 창만 닫습니다. 실제로는 그 값을 하루짜리 저장소에 적고 다음 방문 때 이 컴포넌트를 렌더하지 않도록 화면에서 판단합니다.
- 확인 방법: [기업 메인 공지사항 팝업](/corp/home-notice-popup) `완료` · [기관 메인 공지사항 팝업](/org/home-notice-popup) `완료`
...
- 적용: 지정한 파일만 교체합니다. 위 [신규 추가] `메인 공지사항 팝업` 과 함께 반영합니다.
- 변경: `/corp/home-notice-popup`·`/org/home-notice-popup` 를 더합니다. 빠지면 홈과 같은 화면인데 그 두 경로만 기본 테마로 떨어집니다.
- 전달본: `handoff/theme-routes.ts` 는 그대로입니다 — 전달본에서는 팝업이 홈 위에 뜨는 레이어라 별도 경로가 없습니다(전체메뉴와 같은 이유).

### 퍼블리싱 인덱스 — 신용정보 활용체제·메인 공지사항 팝업 완료 처리

- 대상: src/content/publishing-guide/publishing-index.json
- 적용: 지정한 파일만 교체합니다.
- 변경: 이번에 연 네 화면(기업·기관 신용정보 활용체제, 기업·기관 메인 공지사항 팝업)의 UIUX 상태를 대기중에서 완료로 바꿉니다.
- 유지: 응용2 상태와 IA 구조는 그대로입니다.
