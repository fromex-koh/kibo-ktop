...
### 퍼블리싱 인덱스 — 응용2팀 탄소중립 다운로드·확인서 화면 상태 반영

- 대상: src/content/publishing-guide/publishing-index.json
- 적용: 응용2팀이 올린 상태값을 그대로 받은 것이라 파일을 통째로 덮어씁니다.
- 변경: 탄소중립 선도기업의 다운로드·확인서 화면 7개에 응용2 상태 `완료` 가 새로 붙었습니다. 값이 없던 화면에 붙은 것이고 이미 있던 값을 고친 것은 없습니다.
    - 자가진단 > 결과 확인 — 결과 확인서(다운로드)
    - 선도기업 신청 1·2·3차 > 신청결과 확인 — 신청서(다운로드) · 결과 확인서
- 진척률: 탄소 응용2 진척률이 89%(67/75)에서 99%(74/75)로 올랐습니다. 기업·기관 화면 수치는 그대로입니다.
