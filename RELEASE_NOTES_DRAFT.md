...
### 퍼블리싱 인덱스 — 마이페이지 화면 · 모달 등록과 상태 갱신
...
- 적용: 이번에 전달하는 화면을 등록하고 진행 상태를 갱신한 파일로 교체
- 등록: 화면 13건을 새로 넣었다 — 대표자 역량 입력 도움말 3건(Tech-Index 일반 · 창업 · 투자모형), 기업 [내 정보]의 수정 취소 · 저장 확인 모달, 대표자 이력의 입력 도움말 · 수정 취소 · 저장 확인 모달, 기관 [내 정보] 회원 유형 3종과 그 수정 취소 · 저장 확인 모달
- 구조 변경: 기관 [내 정보]는 한 항목이던 `org-mypage-profile-edit` 를 회원 유형 3건(`-partner-bank` · `-partner-agency` · `-sub-account`)으로 나눴다. 유형마다 묻는 칸이 달라 한 화면으로 묶어 둘 수 없다 — 기존 키를 참조하는 곳이 있다면 함께 바꿔야 한다
- 상태 갱신: 마이페이지 화면 일체와 1:1 문의 목록 · 상세, 이용약관 · 개인정보 처리방침(기업 · 기관), 기관 로그인 안내를 완료로 올렸다
- 순서: 세 파일은 서로 키를 맞춰 검사하므로 함께 받아야 한다. 한쪽만 반영하면 화면이 뜨지 않는다

## [덮어쓰기]

### 기관 기업정보 구획 — 되읽는 안내 문구 삭제

- 대상: src/components/composite/org-company-info-form.tsx
- 적용: [기업정보 관리] 의 노출 조건 안내를 지운 파일로 교체. 버튼과 입력 칸, 나머지 화면 구성은 종전과 같다
- 이유: 시안 메모가 "기업·기술정보 입력 > 기업정보 화면에서 노출" 인데, 이 구획이 놓이는 자리가 세 모형 모두 바로 그 화면이다 — 보고 있는 화면을 조건으로 되읽는 문장이라 읽는 사람에게 새로 알려 주는 것이 없다
- 남는 안내: [기업 자가진단 결과보기] 의 노출 조건 하나뿐이라 목록이 아니라 한 줄로 그린다. 그 버튼이 보이지 않는 화면에서는 안내도 나오지 않는다. 노출 조건이 실제 동작으로 붙으면 이 안내도 걷어낸다
