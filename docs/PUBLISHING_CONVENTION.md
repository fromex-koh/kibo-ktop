...
- **[PB-07] `typo-*` 복합 유틸리티 사용(MUST)** — 제목과 본문은 컴포넌트 가이드의 타이포그래피 목록에 정의된 `typo-*` 클래스를 사용한다. 크기·굵기·행간·자간과 반응형 전환이 함께 적용된다. (예외: 메인페이지 풀스크린 목업의 제목은 해상도별 크기를 위해 반응형 `text-*` 허용 — [SHADCN.md 타이포 유틸 예외](SHADCN.md#타이포-유틸-예외).)
...
- **[PB-18] CSS 파일의 위치(MUST)** — `src/app/globals.css` 에는 전역 설정(Tailwind 변형·`@theme`·base·전역 유틸리티)과 여러 화면이 공유하는 컴포넌트 규칙만 둔다. 한 화면에서만 쓰는 CSS 는 `src/styles/<화면>.css` 로 분리하고 `globals.css` 가 아니라 **그 화면의 `page.tsx` 에서 직접 import** 한다 — 전역에 물리면 다른 화면 번들에도 실린다. 현재 이 디렉터리는 비어 있다(메인페이지 CSS 는 전부 유틸리티·theme variants 로 이관됨) — 새로 만들기 전에 아래 순서를 먼저 밟는다. 파일 맨 위에 `@reference '../app/globals.css';` 를 두어 토큰·변형만 참조하고 출력은 늘리지 않는다. 화면 CSS 안의 규칙이 재사용 컴포넌트의 것이면 CSS 로 남기기 전에 `theme/<컴포넌트>.variants.ts` 의 className 상수로 옮길 수 있는지 먼저 본다([SC-04]) — 조건이 복잡해도 `@custom-variant` 를 등록하면 대부분 표현된다(예: `theme/stack-pager.variants.ts`). 유틸리티로 표현할 수 없을 때만 `src/styles/<컴포넌트>.css` 로 분리한다(높이 기준 분기·조상 상태·형제 순서까지 변형으로 표현된 예: `theme/hero-section.variants.ts`·`theme/hero-stats-roller.variants.ts`). 한 기능의 클래스·`@keyframes`·`prefers-reduced-motion` 예외는 한 블록에 모으고, 각 블록 머리에 `사용처: <파일>` 을 적는다 — 전역 클래스는 IDE 가 참조를 찾아주지 못해 사용처 표기가 없으면 삭제 가능 여부를 판단할 수 없다.
- **[PB-17] 높이 기준 분기는 등록된 변형 사용(MUST)** — Tailwind 기본 브레이크포인트는 너비만 다룬다. 화면 높이로 갈라야 하는 레이아웃(풀스크린 전환 화면 등)은 `[@media_(min-height:40rem)]:` 같은 arbitrary variant 를 사용처마다 적지 말고 등록된 `pager-on:`·`pager-off:`·`short:`·`landscape:`·`stack-fallback:` 을 사용한다. 임계값의 단일 소스는 `tokens.json` 의 `breakpoint.md` 와 `breakpointHeight` 이며, `yarn tokens` 가 `@custom-variant` 를 생성한다 — 조건을 바꾸려면 그 숫자만 고친다. JS 에서 같은 조건을 쓰는 곳(`stack-pager.tsx` 의 `STACK_PAGER_QUERY`)이 어긋나면 생성기가 빌드를 세운다.
