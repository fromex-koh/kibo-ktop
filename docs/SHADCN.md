...
shadcn/ui의 `radix-nova` 레지스트리 스타일을 이 프로젝트의 **디자인 토큰 시스템에 녹여서** 사용한다. primitive는
대부분 `radix-ui`를 사용하며, 레지스트리 원본이 Base UI로 제공되는 Combobox는 `@base-ui/react`를 사용한다.
핵심: shadcn 컴포넌트가 참조하는 **표준 색 슬롯(`--color-*`)을 `tokens.json` 단일 소스**에 연결하고,
컴포넌트 **셸(구조·동작·접근성)은 업스트림 원형을 유지**하며, **프로젝트 스타일 정의(CVA·className 상수)는
`theme/` 레이어**에서 관리한다.
...
- **[SC-01] 기본값은 토큰/기존 유틸리티, arbitrary value 최소화 (MUST)**
  ...
  `calc()`·`min()`·`max()`·`clamp()` 는 토큰 변수를 조합할 때만 허용되고(고정 숫자만 감싸면 금지), 아래
  [arbitrary value 예외 정책](#arbitrary-value-예외-정책)을 함께 적용한다.
  (연계: [PB-01]·[PB-13]·[NA-009]·[ST-004]·[CD-002])

- **[SC-02] shadcn 셸(ui/)의 구조·동작·접근성 수정 금지 — 스타일 추출만 허용 (MUST)**
  `src/components/ui/<name>.tsx` 는 `npx shadcn@latest add` 로 받은 **업스트림 셸**이다. DOM 구조·props·상호작용·
  ...
  만든 컴포넌트의 theme 스타일도 shadcn 원본이 아니므로 vendor 대상이 아니다.
    ...
    바꾸고 `PROJECT-CONVENTION` 주석에 원본을 함께 기록한다. DOM·props·동작은 바꾸지 않으며, 같은 예외를 임의로
    늘리지 않는다.

- **[SC-04] 공통 프로젝트 스타일은 `theme/*.variants.ts` 에서 관리 (MUST)**
  화면·도메인 코드는 `@/components/ui/<name>` 을 **직접 import** 한다(별도 창구 레이어 없음). 컴포넌트의
  색·크기·라운드 등 반복되는 프로젝트 스타일은 **`src/components/theme/<name>.variants.ts`의 CVA 또는 className
  상수**에서 관리한다. 셸(ui)이나 사용처에서 className 덧씌우기를 반복하지 않는다. 일회성 레이아웃 조정은 사용처
  `className`으로 가능하지만, 반복되면 theme variant·공통 상수 또는 composite로 승격한다.
...
| `calc()`·`min()`·`max()`·`clamp()` | **토큰 변수를 조합할 때 허용** — `var(--…)` 또는 `--spacing(n)` 을 하나라도 참조하면 통과 | 고정 숫자만 감싼 계산식은 금지(`pt-[calc(12px+4px)]`) — `check:conventions` 의 `SC-01-calc` 가 차단 |
...
- **`app/**`** — 화면 고유 구조·실제 미디어 비율은 제한적으로 허용하되, 다른 화면에서도 재사용되면 즉시
...
```
 src/components/ui/<name>.tsx              ← 셸 (업스트림 원형 — 구조·동작·접근성·업데이트 담당)
   · 기본: npx shadcn@latest add로 받은 순정 그대로
   · 재스킨한 컴포넌트만: CVA/className 스타일 정의 → theme import 로 교체 ("수정 셸")
   · 화면·도메인 코드가 직접 import 하는 앱-대면 레이어
          │ import {xxxVariants} from '@/components/theme/<name>.variants'
          ▼
 src/components/theme/<name>.variants.ts   ← 프로젝트 스타일 (CVA·className — 색·크기·라운드·타이포)
...
> 수정하거나 재구현하지 않는다** — 그건 전부 **업스트림 셸과 기반 primitive의 책임**이다. (웹접근성 중 순수
...
- **업데이트가 안전하다.** 셸은 업스트림 코드와 구조가 같고 차이는 theme 스타일 import와 명시된 최소 예외뿐이라, 새 버전을 받아
  `git diff` 하면 업스트림 변경분이 그대로 보인다. 스타일은 theme 에 격리돼 있어 그 diff 에 섞이지 않는다.
- **표준 슬롯을 최대한 공유한다.** theme 스타일에서도 `bg-primary`·`text-primary-foreground`·`border-input`·
  `ring-ring` 처럼 shadcn 이 이미 가진 슬롯으로 표현 가능한 값은 그대로 쓴다. 프로젝트 전용 토큰은 표준 슬롯으로
  흡수할 수 없을 때만 남긴다.
- **왜 스타일을 감싸지(compose) 않고 교체하나.** 셸 밖에서 `cn` 으로 덮어쓰는 wrapper 방식은 원본의 상태 유틸과 프로젝트
  상태 유틸(예: `hover:bg-primary/80` vs `hover:bg-primary-hover`)이 twMerge 에서 안정적으로 중복제거된다고
  보장하기 어렵다. 색·사이즈를 전면 재스킨하는 이 프로젝트에선 **원본 스타일 정의를 프로젝트 theme 정의로 교체**하는 것이
  충돌 없는 방식이다(업계에서 heavy customization 시 쓰는 소스 fork 방식의 변형).

### `PROJECT-STYLE` 주석 포맷

theme 스타일에서 shadcn 바닐라와 다른 값을 남겨야 할 때는 코드 근처에 `PROJECT-STYLE:` 주석을 붙인다.
...
```
 src/components/ui/<name>.tsx          (L0) shadcn 셸 — 순정 또는 수정 셸(theme import)
   +  src/components/theme/<name>.variants.ts   프로젝트 스타일(CVA·className)
        │
        ▼
...
- **`custom/` 에 두는 것** — shadcn primitive를 사용하지 않는 자체 구현. `icon`·`list-marker`·`publishing-index`·
  차트류에 더해, 화면 전용 요소(`hero-section`·`stack-pager` 등)와 가이드/데모 전용 요소(`code-block`·`copy-chip`·
  `guide-page-shell` 등)도 여기 둔다. provider(`theme-provider`)만 `src/components` 최상위에 둔다.
...
게이트에서 면제한다. 단 **면제 대상은 "순정" 파일뿐**이고, 스타일을 추출한 **수정 셸은 프로젝트가 유지보수하는
...
3. **재스킨이 필요하면 스타일 정의를 추출한다**:
    - 셸의 바닐라 CVA·className 정의를 `vendor/shadcn-baseline/<name>.variants.ts` 로 옮긴다. export 이름에는
      `shadcn` 접두사를 붙이고 내용은 수정하지 않는다.
    - `src/components/theme/<name>.variants.ts` 를 만들고 프로젝트 스타일 정의를 작성한다. 바닐라와 다른 지점엔
      `PROJECT-STYLE` 주석.
...
1. `npx shadcn@latest add <name> --overwrite` — 셸이 새 순정으로 덮인다.
2. `git diff src/components/ui/<name>.tsx` — 업스트림 셸 변경을 확인한다. 재스킨된 컴포넌트라면 새 셸의 스타일 정의를
   다시 추출한다(theme import 복원). 이때 **새 바닐라 스타일 정의**를 `vendor/shadcn-baseline/<name>.variants.ts` 에
   덮어쓴다.
