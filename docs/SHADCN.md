...
- **`custom/` 에 두는 것** — shadcn primitive를 사용하지 않는 자체 구현. 현재 `icon`·`list-marker`·
  `publishing-index`가 해당한다. provider(`theme-provider`)는 `src/components` 최상위, 가이드 전용 요소는 `guide/`에 둔다.
