...
> **구조 변천 메모** — 과거에는 `ui/`(순정 원본) + `kit/`(styled copy·facade 창구) 2중 레이어였으나,
> 2026-07 에 `kit/` 을 `ui/` 로 통합하고 스타일을 `theme/*.variants.ts` 로 추출하는 현재 구조로 전환했다
> (커밋 `3316ff8`). facade 보일러플레이트·컴포넌트당 2파일·창구 혼동을 없애면서, "셸은 업스트림 /
> 스타일은 프로젝트" 책임 분리는 그대로 유지한다. 이 문서보다 오래된 자료(kit 언급)는 이 메모를 기준으로 읽는다.
