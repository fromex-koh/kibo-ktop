...
- `.github/workflows/release-main.yml`은 릴리스 규칙에 따라 다음 버전을 계산한다. 현재 전환에서는 `v0.1.10` 다음을 `v1.0.0`으로 만들고, 이후 PATCH가 9에 도달하면 MINOR를 올린다.
...
`work`를 `main`에 공유하면 GitHub Actions가 `yarn verify`를 실행하고, 성공한 경우 `scripts/resolve-release-version.mjs`로 다음 릴리스 버전을 계산한다.

```bash
v0.1.8 → v0.1.9 → v0.2.0
v0.1.10 → v1.0.0 → v1.0.1
v1.0.1 → v2.0.0 → v2.0.1
v2.0.8 → v3.0.0 → v3.0.1
```

- PATCH는 `0`부터 `9`까지만 사용한다.
- `v0.1.9` 다음은 `v0.2.0`이며, `v1.0.9` 다음은 `v1.1.0`이다.
- `v0.1.10`은 기존 태그 이력을 정리하기 위한 전환 기준으로만 사용하고, 다음 `main` 릴리스에서 `v1.0.0`으로 전환한다.
- `v1.0.1`은 두 번째 정식 배포의 전환 기준으로 사용하고, 다음 `main` 릴리스에서 `v2.0.0`으로 전환한다.
- `v2.0.8`은 세 번째 정식 배포의 전환 기준으로 사용하고, 다음 `main` 릴리스에서 `v3.0.0`으로 전환한다.
...
- MAJOR·MINOR 변경은 커밋 type 자동 분석이 아니라 이 저장소의 릴리스 증가 규칙에 따라 처리한다.
